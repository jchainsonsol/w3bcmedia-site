...
let obsRetryEnabled = true;
let lastObsError = 'OBS not connected';
...
function scheduleOBSReconnect(delay=5000) {
  if (!obsRetryEnabled) return;
  clearTimeout(reconnectTimer);
  reconnectTimer = setTimeout(connectOBS, delay);
}

function connectOBS() {
  if (obs && (obs.readyState === WebSocket.OPEN || obs.readyState === WebSocket.CONNECTING)) return;
  identified = false;
  try { obs = new WebSocket(OBS_URL); }
  catch (e) { lastObsError=e.message; scheduleOBSReconnect(); return; }
  obs.on('message', raw => {
    ...
    if (msg.op === 0) {
      ...
      if (hello.authentication) {
        if (!OBS_PASSWORD) {
          lastObsError='OBS authentication required; bridge and Elgato remain available.';
          obsRetryEnabled=false;
          console.log('OBS offline: authentication required. Elgato bridge remains available.');
          try { obs.close(); } catch {}
          return;
        }
  ...
  obs.on('close', () => { identified=false; if (obsRetryEnabled) scheduleOBSReconnect(); });
  obs.on('error', err => { lastObsError=err.message; });
}

function obsRequest(requestType, requestData = {}) {
  return new Promise((resolve, reject) => {
    if (!obs || obs.readyState !== WebSocket.OPEN || !identified) return reject(new Error(lastObsError || 'OBS not connected'));
...
function dnsSd(args, timeout=2600) {
  return new Promise((resolve, reject) => {
    ...
    const timer=setTimeout(()=>{ try { child.kill('SIGTERM'); } catch {} done(out); }, timeout);
    child.stdout.on('data', d => out += d.toString());
    child.stderr.on('data', d => err += d.toString());
    child.on('error', e => { if(settled)return;settled=true;clearTimeout(timer);reject(e); });
    child.on('close', code => { if(settled)return; if (out) done(out); else if (code && err) {settled=true;clearTimeout(timer);reject(new Error(err.trim()));} else done(out); });
  });
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (url.pathname === '/health') {
    ...
    return res.end(JSON.stringify({ok:true,bridge:true,elgatoBridge:true,elgatoDiscovery:process.platform==='darwin',obsConnected:identified,obsOptional:true,obsError:identified?null:lastObsError}));
  }
  if (url.pathname === '/scene' && (req.method === 'POST' || req.method === 'GET')) {
    let scene = url.searchParams.get('name');
    if (req.method === 'POST' && !scene) { const chunks=[]; for await (const c of req) chunks.push(c); try { scene = JSON.parse(Buffer.concat(chunks).toString() || '{}').sceneName; } catch {} }
    ...
    try { await obsRequest('SetCurrentProgramScene', {sceneName:scene}); res.writeHead(200, {'Content-Type':'application/json'}); return res.end(JSON.stringify({ok:true,scene})); }
    catch(e){ res.writeHead(503, {'Content-Type':'application/json'}); return res.end(JSON.stringify({ok:false,error:e.message,obsConnected:false})); }
  }
  if (url.pathname === '/elgato/discover' && req.method === 'GET') {
    try { const found=await discoverElgato(); res.writeHead(200, {'Content-Type':'application/json'}); return res.end(JSON.stringify({ok:true,...found})); }
    catch(e){ res.writeHead(404, {'Content-Type':'application/json'}); return res.end(JSON.stringify({ok:false,error:e.message})); }
  }
  if (url.pathname === '/elgato' && (req.method === 'GET' || req.method === 'POST')) {
    let host=url.searchParams.get('host'), state=null;
    if(req.method==='POST'){const chunks=[];for await(const c of req)chunks.push(c);try{const body=JSON.parse(Buffer.concat(chunks).toString()||'{}');host=body.host||host;state=body.state||{}}catch{}}
    ...
    try{const data=req.method==='GET'?await elgatoRequest(host):await elgatoRequest(host,'PUT',{numberOfLights:1,lights:[state||{}]});res.writeHead(200,{'Content-Type':'application/json'});return res.end(JSON.stringify({ok:true,host,data}))}
    catch(e){res.writeHead(503,{'Content-Type':'application/json'});return res.end(JSON.stringify({ok:false,error:e.message}))}
  }
  res.writeHead(404, {'Content-Type':'application/json'}); res.end(JSON.stringify({ok:false,error:'Not found'}));
});

server.on('error',e=>console.error('Bridge HTTP error:',e.message));
server.listen(HTTP_PORT,'127.0.0.1',()=>{
  console.log(`W3BC Local Bridge: http://127.0.0.1:${HTTP_PORT}`);
  console.log('Elgato control: READY (OBS is optional)');
  connectOBS();
});